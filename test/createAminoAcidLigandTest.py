...
def createConformation(model, system):
    ...
    if np.max(np.abs(forces)) > 1000:
        raise Exception('Failed to minimize')
    return simulation.context.getState(getPositions=True).getPositions(asNumpy=True).value_in_unit(unit.nanometer)

def saveConformations(outputfile, ligandName, aaName, conformations, mol):
...
ligandPDB = {}
for line in open('../amino-acid-ligand/cc-to-pdb.tdd'):
    line = line.upper()
    pdbs = line.split()[2:]
    if len(pdbs) >= 5:
        ligandPDB[line[:3]] = pdbs

skip = defaultdict(list)
with h5py.File('/Users/peastman/temp2/SPICE-2.0.1.hdf5') as f:
    for group in f:
        if f[group]['subset'].asstr()[0] == 'SPICE Amino Acid Ligand v1.0':
            skip[group[:3]].append(group[4:])
...
def processLigand(smiles, resid):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        errors = 0
        for pdbid in ligandPDB[resid]:
            try:
                urllib.request.urlretrieve(f'https://files.rcsb.org/download/{pdbid}.cif', pdbPath)
            except:
                continue
            pdb = app.PDBxFile(pdbPath)
            ...
            for residue in pdb.topology.residues():
                if residue.name == resid:
                    ...
                    for neighbor in neighbors:
                        if neighbor.name in skip[resid]:
                            continue
                        try:
                            ...
                            if len(conformations[key]) == 10:
                                return residue.name, neighbor.name, conformations[key], mols[key]
                        except:
                            # This happens when model.pdb and ligand.pdb don't have identical sets of heavy atoms,
                            # or the model can't be minimized.
                            errors += 1
                            if errors == 5:
                                return None, None, None, None
                            pass
...
last = int(sys.argv[2])
outputfile = h5py.File('amino-acid-ligand-test.hdf5', 'w')
for line in open('../amino-acid-ligand/Components-smiles-oe.smi'):
    smiles, resid, name = line.split('\t')
    resid = resid.upper()
    if resid not in ligandPDB:
        continue
    if resid not in skip:
        continue
    ...
    if mol.GetNumAtoms() < first or mol.GetNumAtoms() > last:
        continue
    print(resid, mol.GetNumAtoms(), len(ligandPDB[resid]))
    try:
        residueName, neighborName, conformations, mol = processLigand(smiles, resid)
    except:
        continue
    if conformations is not None:
        saveConformations(outputfile, residueName, neighborName, conformations, mol)
        if len(outputfile) == 200:
            exit()
